...
export default async function DetalleAsociadoPage({
  params
}: {
    ...
    <div className="animate-fade-in" style={{ padding: '1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        <div>
          ...
          <h1 style={{ marginBottom: '0.5rem' }}>Perfil: {asociado.nombre}</h1>
          <p style={{ color: 'var(--text-muted)' }}>Tipo: {asociado.tipo || 'No especificado'}</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '2rem' }}>
        {/* Formulario / Edición */}
        <div>
          <AsociadoForm asociado={asociado} actionType="update" />
        </div>

        {/* Panel lateral: Casos del asociado */}
        <div>
          <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: 'var(--radius-md)' }}>
            ...
            {asociado.cases && asociado.cases.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {asociado.cases.map((c: any) => (
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', textAlign: 'center', padding: '1rem 0' }}>
                Este asociado no tiene casos derivados.
              </p>
            )}
          </div>
        </div>
